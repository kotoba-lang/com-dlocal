(ns dlocal.main
  "Kotodama WASM entrypoint for the Dlocal clean-room actor (L4) — Clojure port.
  ...
  facts (`dlocal.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dlocal")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"dlocal.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dlocal-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dlocal-compat/ready)
